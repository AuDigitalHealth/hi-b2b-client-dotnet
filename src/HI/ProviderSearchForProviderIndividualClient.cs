...
namespace Nehta.VendorLibrary.HI
{
    ...
    /// </summary>
    public class ProviderSearchForProviderIndividualClient : IProviderSearchForProviderIndividualClient
    {
        ...
        public ProviderSearchForProviderIndividualClient(Uri endpointUri, ProductType product, QualifiedId user, QualifiedId hpio, X509Certificate2 signingCert, X509Certificate2 tlsCert, Action<ServiceEndpoint> initialisationCallback)
        {
            Validation.ValidateArgumentRequired("endpointUri", endpointUri);
        ...
        /// <param name="tlsCert">Certificate for establishing TLS connection to the service.</param>
        /// <param name="initialisationCallback">Callback for additional configuration after creation.</param>
        public ProviderSearchForProviderIndividualClient(string endpointConfigurationName, ProductType product, QualifiedId user, QualifiedId hpio, X509Certificate2 signingCert, X509Certificate2 tlsCert, Action<ServiceEndpoint> initialisationCallback)
        {
            Validation.ValidateArgumentRequired("endpointConfigurationName", endpointConfigurationName);
        ...
        /// </summary>
        public async Task<searchForProviderIndividualResponse> ProviderIndividualSearchAsync(searchForProviderIndividual request)
        {
            var envelope = new searchForProviderIndividualRequest()
            {
                searchForProviderIndividual = request,
            ...
            try
            {
                response1 = await providerSearchForProviderIndividualClient.searchForProviderIndividualAsync(envelope);
            }
            catch (Exception ex)
            {
                // Catch generic FaultException and call helper to throw a more specific fault
