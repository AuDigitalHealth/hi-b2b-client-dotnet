...
namespace Nehta.VendorLibrary.HI
{
    ...
    /// </summary>
    public class HIEndpointProcessor6
    {
        private const string UserNamespace = "http://ns.electronichealth.net.au/hi/xsd/common/CommonCoreElements/3.0";
        private const string SecurityNs = "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd";
        private const string SoapAddressNs = "http://www.w3.org/2005/08/addressing";
        ...
        internal class MessageInspector : IClientMessageInspector
        {
            ...
            internal Message SignBodyAndAddressingHeaders(Message requestMessage, ref string outboundXml)
            {
                ...
                nm.AddNamespace("a", SoapAddressNs);
                nm.AddNamespace("h", UserNamespace);
                nm.AddNamespace("x", SecurityNs);
                ...
                if (references.Count > 0)
                {
                    ...
                    XmlElement headerSignatureElement = (XmlElement)root.SelectSingleNode("//s:Header/x:Security", nm);
            ...
            internal static bool VerifyXML(string xml, X509Certificate2 signingCertificate)
            {
                ...
                xmlnm.AddNamespace("def", XmldsigNs);
                xmlnm.AddNamespace("h", UserNamespace);
                xmlnm.AddNamespace("x", SecurityNs);

                XmlElement ele = (XmlElement)env.SelectSingleNode("/s:Envelope/s:Header/x:Security/def:Signature", xmlnm);
