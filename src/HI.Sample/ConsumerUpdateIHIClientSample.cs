...
namespace Nehta.VendorLibrary.HI.Sample
{
    ...
    /// </summary>
    class ConsumerUpdateIHIClientSample
    {
        public void Sample()
        {
            // Set up the request
            ConsumerUpdateIHIClient client = CreateClient();

            // Set up the request
            updateIHI request = new updateIHI();
            ...
            request.name = new NameType[1];
            request.name[0].givenName = new[] { "Jessica", "Anne" };
            request.name[0].familyName = "Wood";

            try
            {
                // Invokes a basic search
                updateIHIResponse ihiResponse = client.UpdateIhi(request);
            }
        ...
        public async void SampleAsync()
        {
            ...
            {
                // Invokes a basic search
                updateIHIResponse ihiResponse = await client.UpdateIhiAsync(request);
            }
            catch (FaultException fex)
            {
                string returnError = "";
        ...
        public ConsumerUpdateIHIClient CreateClient()
        {
            // ------------------------------------------------------------------------------
            // Set up
            // ------------------------------------------------------------------------------
            ...
            var client = new ConsumerUpdateIHIClient(
